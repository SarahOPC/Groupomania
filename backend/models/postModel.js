...
const sqlRequests = {
    sqlGetAllPosts : "SELECT *, (SELECT avatar FROM users WHERE users.id = posts.userId) AS avatar, (SELECT role FROM users WHERE users.id = posts.userId) AS role, (SELECT value FROM likesdislikes WHERE likesdislikes.userId = ? AND likesdislikes.postId = posts.id) AS liked FROM posts",
